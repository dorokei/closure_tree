module ClosureTree
  module NumericOrderSupport
    ...
    module MysqlAdapter
      def reorder_with_parent_id(parent_id, minimum_sort_order_value = nil, grouped_by_column_id = nil)
        min_where = if minimum_sort_order_value
          "AND #{quoted_order_column} >= #{minimum_sort_order_value}"
        else
          ""
        end
        grouped_where = if grouped_by_column_id
          "AND #{grouped_by_column_name} = #{grouped_by_column_id}"
        else
          ""
        end
        ...
        connection.execute <<-SQL.strip_heredoc
          ...
          WHERE #{where_eq(parent_column_name, parent_id)} #{min_where} #{grouped_where}
    ...
    module GenericAdapter
      def reorder_with_parent_id(parent_id, minimum_sort_order_value = nil, grouped_by_column_id = nil)
        ...
        if grouped_by_column_id
          scope = scope.where("#{grouped_by_column_name} = #{grouped_by_column_id}")
        end
